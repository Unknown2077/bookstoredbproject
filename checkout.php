...
if(isset($_SESSION['id'])){
	...
	while ($row = $result->fetch_assoc()) {
		...
		if ($existingOrders->num_rows > 0) {
			$existingRow = $existingOrders->fetch_assoc();
...
<?php
	...
	echo "<tr><td style='background-color: #ccc;'></td><td style='text-align: right;background-color: #ccc;''>Total Price: <b>IDR".$total."</b></td></tr>";
	echo "</table>";
	echo "</div>";

	$sql = "UPDATE `orders` SET Status = 'y' WHERE CustomerID = ".$cID."";
	$conn->query($sql);
}
