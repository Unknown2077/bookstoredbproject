...
while ($row = $result->fetch_assoc()) {
    // Cek apakah BookID sudah ada di tabel order dengan status 'N'
    $checkOrder = "SELECT * FROM `order` 
                   WHERE CustomerID = ".$row['CustomerID']." 
                   AND BookID = '".$row['BookID']."' 
                   AND Status = 'N'";
    $existingOrder = $conn->query($checkOrder);

    if ($existingOrder->num_rows > 0) {
        // Jika ada, update Quantity dan TotalPrice
        $existingRow = $existingOrder->fetch_assoc();
        $newQuantity = $existingRow['Quantity'] + $row['Quantity'];
        $newTotalPrice = $existingRow['TotalPrice'] + $row['TotalPrice'];
        ...
        $insertOrder = "INSERT INTO `order` (CustomerID, BookID, DatePurchase, Quantity, TotalPrice, Status) 
